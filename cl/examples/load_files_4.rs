...
fn main() {
    ...
    let path = Path::new("./tmp/ts_express_node_modules.zip");
    let path_v = fs::read(path).unwrap();
    let total_size = path_v.len() as f64;

    match ocl_fs_2::write(path, path_v.as_slice()) {
        Ok(_) => {
            // pass
        }
        Err(e) => {
