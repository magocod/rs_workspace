...
fn main() -> Result<()> {
    ...
    let h_b = h.as_bytes();
    println!("produce input: {h} -> {h_b:?}");
    produce(&context, &producer_kernel, &queue, &pipe_vec, h_b)?;

    let w = String::from("world");
    let w_b: &[u8] = w.as_bytes();
    println!("produce input: {w} -> {w_b:?}");
    produce(&context, &producer_kernel, &queue, &pipe_vec, w_b)?;
